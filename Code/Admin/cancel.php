...
if($_SERVER['REQUEST_METHOD']=="GET")
{
    ...
    if(!$conn)
    {
        die("sry connection failed due to Some Technical issues");
            ...
            $t=0;
            $id = $_GET['id'];
            $sql="UPDATE `items` SET `Approved` = 'False' WHERE `items`.`ID` = $id";   
            $result = mysqli_query($conn,$sql);

            $sql1 = "SELECT * FROM `items` WHERE `ID`!=$id AND `Approved` Is Null";
                ...
                while($row = mysqli_fetch_array($result1))
                {
                  ...
                  ?>
                   ...
                   <td><?php echo $row['Institute ID']; ?></td>
                    ...
                    <td><a href='approve.php?id=<?php echo $row['ID'];?>'><img src='../Images/approve.png' width='20px'></a></td>
                    <td><a href='cancel.php?id=<?php echo $row['ID']?>'><img src='../Images/cancel.png' width='20px'></a></td>
